...
1. Если у вас нет аккаунта на GitHub, нужно будет зарегистрироваться там.

1. Установите и настройте себе Git, используя инструкции например [отсюда][help-git].

1. Для этого курса нам потребуется Java 11 версии: Java SE Development Kit 8u181, которую можно скачать со [следующей страницы][java-download]. Будьте внимательны, нам потребуется именно Java 11, с другими версиями проект не заработает.

1. Затем следует установить систему сборки [Gradle][gradle] версии 5.1.1, и добавить путь до её бинарника в `PATH`.
